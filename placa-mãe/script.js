...
import { placaMae } from "./data/data.js";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    placaMae.forEach((placaMae) => {
        const row = document.createElement("tr");
        row.classList.toggle('active');

        row.innerHTML = `
            <td><img src="${placaMae.image}" alt="">${placaMae.nome}</td>
            <td>${placaMae.socket}</td>
            <td>${placaMae.formato}</td>
            <td><strong>$${placaMae.preco.toFixed(2)}</strong></td>
            <td>
            <a href="${placaMae.link}" target="_blank">
